...
def hiveddl(partition, connection, table, name):
    ...
    if len(partition_spec) == 0:
        return None
    return "ALTER TABLE {0} ADD IF NOT EXISTS PARTITION ( {1} )".format(
    ...
    if query is not None:
        try:
            ...
            if exit:
                lock.acquire()
                if returncode == 0:
                    print("파티션 생성 성공: {0}".format(name), flush=True)
                else:
                    print("파티션 생성 실패: {0}".format(name), flush=True)
...
def main():
    if len(sys.argv) < 3:
        print("사용법: hivecreatepartition {배치기준시간} {연결관리번호} [테이블명]...", flush=True)
        sys.exit(1)
    initalize()
    if conf["threads"] > 1:
